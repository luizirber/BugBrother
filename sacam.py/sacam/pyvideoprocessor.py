...
class Videoprocessor(object):
    ...
    def __init__(self, name):
        ...
        self.point_list = []
        self.running = False
        self.detector = gst.element_factory_make("identity")

    def start(self, source, output, project):
        ''' Calculate the insect motion using a threshold difference
            algorithm.
...
#            gobject.source_remove(self.device_manager.timeout_id)
        ...
        while (self.running):
            if self.first_run:
                ...
                self.point_list.append(ptemp)

        return True

    def stop(self, project):
        self.running = False
        project.current_experiment.point_list = self.point_list
